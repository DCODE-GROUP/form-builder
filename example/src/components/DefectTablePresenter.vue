<template>
  <div class="border rounded-xl p-6">
    ...
    <div class="border rounded-lg overflow-hidden divide-y">
      <p class="flex justify-center items-center font-normal text-gray-900 text-sm bg-gray-50 px-4 py-3">
        Defect Type Identified
      </p>
      <div v-for="(row, rowIndex) in rows" :key="rowIndex" class="w-full flex">
        <div class="w-full">
    ...
    <div v-if="data.hasOwnProperty('other_note_label')" class="w-full flex mt-4 border rounded-lg">
      <div class="w-full">
    ...
    <div>
      <div v-for="(defect, index) in data.defects" class="relative rounded-lg border overflow-hidden mt-4">
        <a
        ...
        <div class="flex p-4 border-b">
          <input-wrapper is-vertical field="state" :label-text="defect.description_label" class="w-full">
            <textarea cols="30" rows="3" v-model="defect.description_value" placeholder="Enter your description" />
          </input-wrapper>
        </div>
        <div class="flex p-4">
          // Todo implement file upload here for defect attachments
        </div>
      </div>
      <a
          @click="addRow"
...
const rows = ref(props.field.data.defect_types || []);
const data = ref(props.field.data || []);
